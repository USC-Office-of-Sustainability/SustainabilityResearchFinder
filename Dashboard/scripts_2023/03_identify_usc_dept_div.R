...
# --- Collapse to one row per author, initialize Dept/Div columns -------------
focused_usc_authors <- read.csv("data_processed/02_usc_authors_identified.csv")
...
# Match each author's combined affiliation string against department/school patterns
# (~5 min runtime)
for (i in 1:nrow(focused_usc_authors_dept_div)) {
  ...
  matched_schools <- apply(usc_schools, 1, function(x) {
    if (grepl(x['pattern'], affiliations, ignore.case = TRUE)) x['name']
  })
...
# --- Expand to one row per Dept x Div, validate combinations -----------------
...
usc_departments <- read.csv("data_raw/usc_departments.csv") %>%
  rename(Division = School.Institute.Center, Department = Department.Group)

# Build lookup table that includes an Other row for every Division
...
# Drop Dept/Div combos where the Dept exists in the reference but not under this Div
focused_usc_authors_dept_div_separate_fixed <- merge(
  focused_usc_authors_dept_div_separate, usc_departments_other,
  by.x = c("Dept", "Div"), by.y = c("Department", "Division"),
  all.x = TRUE
)
focused_usc_authors_dept_div_separate_fixed %>%
  mutate(DeptDivExists = !is.na(id),
         IncorrectDiv  = !DeptDivExists & Dept %in% usc_departments_other$Department) %>%
  filter(!IncorrectDiv) %>%
  select(-DeptDivExists, -IncorrectDiv) -> focused_usc_authors_dept_div_separate_fixed

# Reclassify "Lab" departments as Other
focused_usc_authors_dept_div_separate_fixed[
  grep("Lab", focused_usc_authors_dept_div_separate_fixed$Dept), ]$Dept <- "Other"
...
# Re-clean Other dept/div after manual corrections
focused_usc_authors_dept_div_separate_fixed %>%
  group_by(authorID, Div) %>%
  add_count() %>%
  mutate(hasOther = grepl("Other", Dept)) %>%
...
for (i in 1:nrow(metrans)) {
  ...
  matched_lastname_firstname <- which(startsWith(
    focused_usc_authors_dept_div_separate_fixed[matched_lastname, ]$firstname, metrans$firstname[i]))
...
focused_usc_authors_dept_div_separate_fixed$Div[
  which(focused_usc_authors_dept_div_separate_fixed$Dept == "METRANS Transportation Center")] <- "Other"
focused_usc_authors_dept_div_separate_fixed$Dept[
  which(focused_usc_authors_dept_div_separate_fixed$Dept == "METRANS Transportation Center")] <- metrans$DEPARTMENT_AffiliationAdd[1]
...
# --- Remap historically merged authorIDs -------------------------------------
...
new_authorIDs <- sapply(focused_usc_authors_dept_div_separate_fixed$authorID, function(x) {
  if (as.character(x) %in% separate_past_authorIDs$all_authorIDs) past_authorIDs_map[as.character(x)] else x
})
new_authorIDs <- unname(new_authorIDs)
cat("AuthorIDs remapped:", sum(focused_usc_authors_dept_div_separate_fixed$authorID != new_authorIDs), "\n")
focused_usc_authors_dept_div_separate_fixed$authorID <- new_authorIDs

write.csv(focused_usc_authors_dept_div_separate_fixed,
          "data_processed/03_authors_with_dept_div.csv",
          row.names = FALSE)

# Update bridge table with remapped authorIDs
bridge_table <- read.csv("data_processed/02_bridge_pubid_authorid.csv")
