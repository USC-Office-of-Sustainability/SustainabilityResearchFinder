...
This [Sustainability Research Finder](https://usc-sustainability.shinyapps.io/research-dashboard/){target="_blank"} is a R Shiny dashboard that displays the results of mapping USC affiliated research products to the United Nations 17 Sustainable Development Goals (SDGs). Furthermore, each product is categorized as sustainability focused, sustainability inclusive, SDG related, or not related. The results are used for the [Sustainability Tracking, Assessment & Rating System (STARS)](https://reports.aashe.org/institutions/university-of-southern-california-ca/report/2023-12-20/){target="_blank"} and for metrics used in [USC's Assignment Earth Goals](https://sustainability.usc.edu/2028-framework/){target="_blank"} 2.1 and Inclusion goal 3.3.

This project was initiated during the 2022 Fall [CKIDS Datafest](https://sites.usc.edu/ckids/about/){target="_blank"} at USC by Dr. Julie Hopper in the Office of Sustainability and five USC students: Alison Chen, Aurora Massari, Bhavya Ramani, Ric Xian and Xinyi Zhang. Feedback was provided by the USC PWG Research Committee and several new iterations and data processing pipelines have been incorporated by Dr. Julie Hopper, Alison Chen and Feiyang Wang since.
...
1. [Clone](https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository){target="_blank"} this repository to open, view, and edit all the files in this repository on your computer (locally).
2. Due to GitHub file size limitations, the complete USC research data could not be completely uploaded to the repository. As a result, the large data files are stored here [Research Finder Complete Files on Google Drive](https://drive.google.com/drive/folders/1VpqSxIkEDJzRyWmG5nHgqypOlre5jN4K?usp=share_link){target="_blank"}. Please download all four folders ("data_manual", data_processed", "data_raw", "shiny_app") and put them in the "Dashboard" folder of the cloned repository. You may be asked whether to replace those folder, and please click "replace", as the folders on Google Drive contain the most complete data. The folder structure should look like this:
...
└── docs
```
3. Open RStudio and click on ![the Project button in RStudio](README_files/RProjectButton.PNG) in the top right to open the [project](https://support.posit.co/hc/en-us/articles/200526207-Using-RStudio-Projects#:~:text=Opening%20Projects,Rproj){target="_blank"} file `Dashboard.Rproj`. This will automatically set the working directory as the project directory.
...
<details>
  <summary>List of packages that need to be installed before running the shiny app</summary>
```{r, eval=FALSE}
...
<details>
  <summary>List of packages that need to be installed before running the data processing pipeline (scripts_2023/)</summary>
```{r, eval=FALSE}
install.packages("dplyr")
...
To run the shiny app, open the `shiny_app/app.R` file in RStudio and click Run App.

### Updating the Data Pipeline

**To update keywords and/or context dependencies:** Add the new files to `data_raw/`. In `scripts_2023/08_run_text2sdg.R`, update the file names in the two read.csv() functions. Then run `08_run_text2sdg.R` and all the following scripts (09, 10, etc.).

**To add new publications from Scopus:** Add the new file to `data_raw/` and then in `scripts_2023/01_read_data.R`, append a new entry to the `NEW_YEAR_FILES` list with the file path and target year. Run all the files in `scripts_2023/` (or use `run_all.R`).
**Important**: Always use the CSV file exactly as downloaded from Scopus — do not open and re-save it in Excel first. Doing so will corrupt special characters in the data. If you need to inspect the file in Excel, use the Data Tab > Get Data > From File > From Text/CSV to import it for viewing, then save a separate copy as an Excel workbook (.xlsx) if needed.

**To add new law publications:** Add the new CSV file to `data_raw/` and append its path to the `LAW_FILES` vector in `scripts_2023/05_add_law_pubs.R`.
...
This README is a [RMarkdown GitHub document](https://rmarkdown.rstudio.com/github_document_format.html){target="_blank"}. To update the README, open and edit the README.Rmd file in RStudio. Then click Knit to generate the new README.md file. Upload/commit both files to GitHub.

### Folder Structure

- Dashboard/ contains all the code from processing the raw data to creating the Shiny dashboard
  - data_raw/ contains all the raw data (publications, keywords, department reference files)
  - data_processed/ contains all the data generated by R scripts
  - data_manual/ contains all the data that was manually edited
  - scripts_2020/ contains legacy code for the original 2020-22 data build (reference only; not rerun)
  - scripts_2023/ contains the active pipeline scripts for processing all data (2020-2025); run these to update the dashboard
  - shiny_app/ contains the code for the R Shiny dashboard and the data files it reads at runtime
  - STARS/ contains the code used to generate summaries for STARS report
- docs/ is the code for the [website](https://usc-office-of-sustainability.github.io/SustainabilityResearchFinder/){target="_blank"} created for CKIDS Datafest 2022
...
- Publication data was downloaded from [Scopus](https://www.scopus.com/){target="_blank"} with the help of Xinyi Zhang.
<details>
  <summary>How we downloaded the data from Scopus</summary>
  <ol>
    <li>Go to [scopus.com](https://www.scopus.com/){target="_blank"} and log in via institution by clicking the building icon in the top right.</li>
    <li>Search in Organization "University of Southern California".<img src="README_files/search_organization.png"/></li>
    <li>Click the number of documents.<img src="README_files/org_search_result.png"/></li>
    <li>Double check the search query by clicking edit.<img src="README_files/search_query.png"/></li>
    <li>Select all documents and export (not download) as CSV. Include all Citation Information, Affiliations, Publisher, and all Abstract & Keywords. Tip: download year by year since there is a limit to the number of documents you can export at one time.<img src="README_files/checklist.png"/></li>
    <li>To view the downloaded csv file in Excel, open Excel, go to the Data Tab > Get Data > From File > From Text/CSV, select the csv and load. However, we recommend not editing the csv in excel since we ran into issues where the special characters in the data got messed up.</li>
  </ol>
</details>
- Additional publications from the Gould School of Law were provided with the help of Amber Kennedy Madole.
<details>
  <summary>How we added publications outside of Scopus</summary>
  <ul>
    <li>Create a Google Sheet with the following columns: Publication Type, Author Name (Last, First), Division, Department, authorID, Title, Abstract, Link, Year, Source Title, Volume, Issue, Article Number, Page start, Page end, Page count, DOI, Open Access. </li>
    <li>Each row is one author with one publication.</li>
    <li>Manually assign authorIDs by searching the author in the existing dataset and/or searching on Scopus. If the author does not exist, assign an arbitrary id such as Gould10.</li>
  </ul>
</details>
...
scripts_2020/ contains to code for processing 2020-22 publications. However, it is not feasible to rerun all the code every time there are new publications. As a result, scripts_2023/ contains code for processing new publications (2023 and beyond). Both sets of scripts have the same idea overall.
...
<details>
  <summary>Using Elsevier API</summary>
  <strong>Prerequisites</strong>
  <ol>
    <li>Create developer account at https://dev.elsevier.com/</li>
    <li>Create an API key</li>
    <li>Email support for an institution token</li>
  </ol>
  <strong>Setting up the API key</strong>
  <ol>
      <li>Create a .Renviron file with </li>
  </ol>
...
<details>
  ...
  <ol>
    <li>Using Chrome, go to [USC directory search](https://uscdirectory.usc.edu/web/directory/faculty-staff/){target="_blank"}. </li>
    <li>Right click on the page and select Inspect.</li>
    <li>In the panel, select Network.</li>
    <li>Search Julie Hopper in the USC Directory Search bar.</li>
    <li>Find the entry in the network where the Request URL looks like <code>https\://uscdirectory.usc.edu/web/directory/faculty-staff/proxy.php?basic=julie%20hopper</code></li>
    <li>Scroll down to the Request Headers section and copy the cookie to your uscdirectory.cookie file.</li>
  </ol>
</details>
...
In scripts_2023/01_read_data.R, we combine all publication data: old 2020-22 data, missing publications from 2020-22, and new Scopus exports for 2023, 2024, and 2025. The script uses a `NEW_YEAR_FILES` config list so that adding another year only requires appending one entry. For each new year file, only records matching the target year are added; when the same publication (matched by Link) already exists from an earlier export, the older row is kept. We filter out document types Letter, Retracted, Note, and Erratum and keep publications with years 2020-2025. pubIDs are propagated from the historical data via EID and new sequential IDs are assigned to any remaining records. The combined data is saved as the publication file.

2. Identify USC authors

Next, we want to identify USC authors in the new data. Most of the code in scripts_2023/02_identify_usc_authors.R is based on scripts_2020/02_identify_usc_authors.R with improvements. The script runs data integrity checks on author field counts and logs any mismatches. When the Authors.with.affiliations column does not match up with the author name column, we use the last name to find the affiliation that starts with the same last name. The 2025 Scopus export format sometimes omits full names for initials-only authors; these are re-aligned by matching the embedded Scopus ID in each full-name entry. Authors whose authorID appears in the historical lookup table but have no detectable USC affiliation in the new data (e.g. initials-only authors) are identified and included. The bridge table is generated using the pubID and authorID columns.
...
Next, we want to combine the new author and bridge files with the old author and bridge files. When combining the author data, we want to keep the first entry but still have the affiliation information. Also, we removed duplicates in the bridge file.

5. Add law publications

scripts_2023/05_add_law_pubs.R is almost exactly the same as scripts_2020/05_add_law_pubs.R for adding new law publications. The script uses a `LAW_FILES` config vector listing the law publication CSV files for each year (currently USC_Law_2023.csv, USC_Law_2024.csv, and USC_Law_2025.csv). All files are combined and filtered to years 2020 and later. We removed law publications that already exist in our dataset (deduplicated by normalized title and year), manually assigned authorIDs in the spreadsheet, assigned pubIDs to the new law publications, and created columns to match the existing author, bridge, and publication files. In addition, all law departments are normalized to "Law".
...
scripts_2023/13_correct_final_div_dept.R applies a manual corrections spreadsheet (data_manual/Research Division and Department Corrections - 2020-2024-Original Dept_Division with corrections.csv) to the author data produced by script 07. It joins on the existing Dept and Div values, replaces them with the corrected names where provided, and removes rows that become identical after renaming. The result is written back to data_processed/07_authors_manual_edited.csv.

scripts_2023/14_apply_dept_mapping.R maintains a persistent mapping table (data_raw/div_dept_mapping.csv) that translates all (Dept, Div) pairs in the author data to canonical values from the reference file data_raw/usc_departments_Mar_30_2026.csv. Each run loads any existing mapping, detects new pairs not yet in the table, and auto-matches them using four strategies in order: exact match on both Dept and Div, exact match on Dept only, regex pattern match, and fuzzy string similarity. Pairs that cannot be matched automatically are flagged with manual_correction_required = TRUE; to resolve them, edit data_raw/div_dept_mapping.csv and set that flag to FALSE, then re-run the script. The corrected author data is saved to shiny_app/14_authors_dept_corrected.csv, which is the file read by the Shiny app.
...
The main file for the [shiny app](https://rstudio.github.io/shinydashboard/){target="_blank"} is shiny_app/app.R. The www/ folder is expected by a Shiny application to contain images and stylesheets.

Our dashboard is hosted on the web with [Shinyapps.io](https://shiny.posit.co/r/articles/share/shinyapps/){target="_blank"}. Also, we did some load testing using Python [Locust](https://locust.io/){target="_blank"}.


## Acknowledgements

This project was initially inspired by Carnegie Mellon's work on mapping its courses to the 17 UN SDGs. For more information, please read about our initial collaboration with Alex Hinicker and Peter Wu in our [Sustainability Course Finder Github Repository](https://github.com/USC-Office-of-Sustainability/SustainabilityCourseFinder){target="_blank"}. This project has been improved by feedback from many students, staff and faculty in [USC's Office of Sustainability](https://sustainability.usc.edu/){target="_blank"} and in [USC's Presidential Working Group on Sustainability in Research](https://www.president.usc.edu/sustainability-pwg/){target="_blank"}.  In particular, we would like to recognize the following people for their contributions to the development of this project, and the data and code: Alison Chen, Amber Madole, Aurora Massari, Bhavya Ramani, Brian Tinsley, Feiyang Wang, Julie Hopper, Ric Xian, and Xinyi Zhang.

## Questions?

If you have any questions, comments, or concerns, please reach out to Dr. Julie Hopper: oosdata(at)usc.edu
