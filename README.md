...
- Publication data was downloaded from
  ...
  <li>
  Search Organization “University of Southern California”.
  </li>
  <li>
  Select all documents and export as CSV. Include all Citation
  Information, Affiliations, Publisher, and all Abstract & Keywords.
  Tip: download year by year since there is a limit to the number of
  documents you can export at one time.
  </li>
  </ol>
  </details>
...
to. I created a spreadsheet with a list of all the departments and
...
authors with no department or division, I combined the authors that had
the same first name, last name, department and division. I also combined
authors that had the same first name, last name, and with one or more
department overlap. Then, I created a spreadsheet of authors still with
the same first and last name. Xinyi and I manually checked the file to
identify the authorIDs that were indeed the same person. For every
combination of authorID, there is an old authorID and a new authorID. I
used a named vector as a dictionary to map old authorIDs to new
authorIDs where the name of the vector is the old authorID. The old
authorID can be checked by `oldID %in% names(dict)` and accessed by
`dict[oldID]`. Finally, we made some manual fixes for duplicate entries
that we noticed.

3.  Get USC author info

Some authors do not have a department since their affiliation lacks
information. As a result, I searched up all the authors without a
department in the
...
target="_blank">RCurl</a> and RJSONIO. I searched by first and last name
and looked through faculty and staff before students. However, if the
result is a person in a different division, I did not include the
information. Since the departments and divisions are written differently
than my list of departments and divisions, I had to fix a lot of
...
and year. In the law publication spreadsheet, I manually assigned the
authors an authorID by searching for their name in the existing data. If
they were not found, I assigned them an ID in the format Gould#. In
scripts_2020/05_add_law_pubs.R, I used the law publication spreadsheet
...
authorIDs. As a result, I went through all the scripts to save all the
...
#### Sustainability in Los Angeles Publications

From our mapped publications, we looked at sustainability focused and
inclusive publications and used a <a
...
impacted. In addition, we ordered the publications such that
